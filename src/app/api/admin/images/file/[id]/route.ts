...
import { readImageRecordFileMetaForAdmin } from "@/lib/server/files";
import {
...
async function getAdminImageFileResponseContext(context: { params: Promise<{ id: string }> }) {
  await requireAdmin();
  const { id } = await context.params;
  const image = await readImageRecordFileMetaForAdmin(id);
  return createImageResponseContext(image);
}

export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const responseContext = await getAdminImageFileResponseContext(context);
